...
def onmessage(update,bot:ObigramClient):
    try:
        ...
        if username in str(tl_admin_user).split(';') or user_info :  # validate user
            ...
            msg323= "🚫No tienes acceso al bot🚫"
            bot.sendMessage(update.message.chat.id,msg323)
            try:
                bot.sendMessage(chat_id=group_id,text=f"EL Usuario: @{username} ha intentado acceder al bot")
            except:pass     
            return
        ...
        if '/add' in msgText:
            ...
            if isadmin:
                try:
                    ...
                    try:
                        bot.sendMessage(chat_id=group_id,text=f"@{user} Es [USUARIO] del bot")
                    except:pass    
                except:
        ...
        if '/evea' in msgText:
            ...
            if getUser:
                ...
                bot.sendMessage(update.message.chat.id,f"✅Configuración Aplicada{hostmo}")
            return
        if '/eva' in msgText:
            getUser = user_info
            user = ''
        ...
        if '/shorturl' in msgText:
            ...
            if isadmin:
                try:
                    ...
                    bot.sendMessage(update.message.chat.id,"⚠️Error al activar el acortador de enlaces⚠️" + str(ex))
            return
        # end
        ...
        if '/acc' in msgText:
            try:
                ...
                bot.sendMessage(update.message.chat.id,'⚠️Error al guardar el usuario y contraseña: '+str(ex))
            return
        if '/host' in msgText:
            try:
                ...
                if "https" in host or "http" in host:
                 ...
                 if getUser:
                    ...
                    jdb.save()
                    statInfo = infos.createStat(username,getUser,jdb.is_admin(username))
                    bot.sendMessage(update.message.chat.id,"☁️Host guardado")
                    bot.sendMessage(chat_id=group_id,text=f"☁️Usuario:{username} ha configurado su host:{host}")
                else: bot.sendMessage(update.message.chat.id,"⚠️Eso no es un url")    
            except Exception as ex:
                bot.sendMessage(update.message.chat.id,'⚠️Error al guardar el host: '+str(ex))
            return
        if '/repo' in msgText:
            try:
                ...
                bot.sendMessage(update.message.chat.id,'⚠️Ponga el repo correctamente :)')
            return
        ...
        if '/rename_on' in msgText:
            try:
                ...
                if getUser:
                    ...
                    jdb.save()
                    statInfo = infos.createStat(username,getUser,jdb.is_admin(username))
                    bot.sendMessage(update.message.chat.id,"Rename activado")
            except: bot.sendMessage(update.message.chat.id,"⚠️No se pudo activar el autonombrado")        
            return
        if '/rename_off' in msgText:
            try:
                ...
                if getUser:
                    ...
                    jdb.save()
                    statInfo = infos.createStat(username,getUser,jdb.is_admin(username))
                    bot.sendMessage(update.message.chat.id,"Rename desactivado")
            except: bot.sendMessage(update.message.chat.id,"⚠️No se pudo descativar el autonombrado")
            return
        if '/nube' in msgText:
            try:
                cmd = str(msgText).split(' ',2)
                ...
                bot.sendMessage(update.message.chat.id,'⚠️Error en el comando /nube (moodle o cloud)')
        if '/up' in msgText:
            try:
                cmd = str(msgText).split(' ',2)
                type = cmd[1]
                ...
                bot.sendMessage(update.message.chat.id,'⚠️Error en el comando /up: '+str(ex))
            return
        if '/proxy' in msgText:
            try:
        ...
        if '/decrypt' in msgText:
            ...
            bot.sendMessage(update.message.chat.id, f'🔌Proxy desencryptado:\n{proxy_de}')
            return
        ...
        if '/dir' in msgText:
            try:
                ...
                bot.sendMessage(update.message.chat.id,'⚠️Error, ponga su carpeta')
            return
        ...
        if '/login' in msgText:
             ...
             if getUser:
                ...
                if user and passw and host != '':
                        ...
                        logins = client.login()
                        if logins:
                                bot.editMessageText(message,"⚙️Conexion Lista")  
                        else: bot.editMessageText(message,"⚙️Error al conectar")                         
        ...
        if '/token' in msgText:
            ...
            try:
             ...
             if loged:
                ...
                bot.editMessageText(message,'⚠️Error. Revise la nube')
             return
        ...
        if '/del_files' in msgText and user_info['cloudtype']=='moodle':
            ...
            try:
                for contador in range(50):
                    ...
                    if loged:               
                        ...
                        bot.sendMessage(update.message.chat.id,'⚠️Error. Revise la nube')
           ...
           try: 
            ...
            bot.editMessageText(message,"⚠️Error al intentar bajar el archivo"+str(ex)) 
    except Exception as ex:
           print(str(ex))
